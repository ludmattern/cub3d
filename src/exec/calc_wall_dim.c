...
/*   Updated: 2024/05/21 11:28:33 by lmattern         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    if (ctx->side == 0)
        ctx->wall_dist = (ctx->map_x - rc->pos_x + (1 - ctx->step_x) / 2) / ctx->ray_dir_x;
    else
        ctx->wall_dist = (ctx->map_y - rc->pos_y + (1 - ctx->step_y) / 2) / ctx->ray_dir_y;
...
{
	ctx->texture_x = (int)(ctx->wall_x * (double)rc->texture[ctx->texture_num].width);
	if (ctx->side == 0 && ctx->ray_dir_x > 0)
		ctx->texture_x = rc->texture[ctx->texture_num].width - ctx->texture_x - 1;
