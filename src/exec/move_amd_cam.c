...
/*   Updated: 2024/05/20 16:34:04 by lmattern         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_forward_backward(t_ray *rc, double ms)
{
	if (rc->move_forward)
	{
		if (rc->map->grid[(int)(rc->pos_y)][(int)(rc->pos_x + rc->dir_x * ms)] == 0)
			rc->pos_x += rc->dir_x * ms;
		if (rc->map->grid[(int)(rc->pos_y + rc->dir_y * ms)][(int)(rc->pos_x)] == 0)
...
void	move_left_right(t_ray *rc, double ms)
{
	if (rc->move_right)
	{
		if (rc->map->grid[(int)(rc->pos_y)][(int)(rc->pos_x + rc->plane_x * ms)] == 0)
			rc->pos_x += rc->plane_x * ms;
		if (rc->map->grid[(int)(rc->pos_y + rc->plane_y * ms)][(int)(rc->pos_x)] == 0)
